GenAlgForSubsetSelectionNoTest<-function (P, ntoselect, npop=100, nelite=5, keepbest=TRUE,tabu=TRUE, tabumemsize=1, mutprob=.8, mutintensity=1, niterations=500, minitbefstop=200,niterreg=5, lambda=1e-6, plotiters = FALSE,plottype=1, errorstat="PEVMEAN2",C=NULL, mc.cores=1,  InitPop=NULL, tolconv=1e-7, Vg=NULL, Ve=NULL, Fedorov=FALSE){
  ...
  if (sum(errorstat %in%c("AOPT", "DOPT", "CDMEAN", "PEVMEAN", "PEVMEAN2"))>0){
   if (Fedorov) {InitPop<-lapply(1:npop, function(q){
     ...
     InitPop<-lapply(1:npop, function(q){
       return(sample(rownames(P),ntoselect))
     })
   }
