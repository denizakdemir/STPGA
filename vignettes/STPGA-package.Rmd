...
eopt_small <- e_optimality(train_small, test_set, Wheat.M)
eopt_medium <- e_optimality(train_medium, test_set, Wheat.M)
eopt_large <- e_optimality(train_large, test_set, Wheat.M)
...
```{r pev-criteria}
# Mean PEV
pev_mean_small <- pev_mean(train_small, test_set, Wheat.M)
pev_mean_large <- pev_mean(train_large, test_set, Wheat.M)

# Normalized PEV (scale-invariant)
pev_norm_small <- pev_mean(train_small, test_set, Wheat.M, normalized = TRUE)
pev_norm_large <- pev_mean(train_large, test_set, Wheat.M, normalized = TRUE)

# Maximum PEV (worst-case scenario)
pev_max_small <- pev_max(train_small, test_set, Wheat.M)
pev_max_large <- pev_max(train_large, test_set, Wheat.M)
...
## Cook's Distance Criteria

Cook's Distance identifies influential observations that significantly affect model predictions.

```{r cook-distance}
# Mean Cook's Distance
cd_mean_small <- cd_mean(train_small, test_set, Wheat.M)
cd_mean_large <- cd_mean(train_large, test_set, Wheat.M)

# Normalized Cook's Distance
cd_norm_small <- cd_mean(train_small, test_set, Wheat.M, normalized = TRUE)
cd_norm_large <- cd_mean(train_large, test_set, Wheat.M, normalized = TRUE)

cat("Cook's Distance Results:\n")
...
P_subset <- Wheat.M[train_mm, 1:100]  # Use only first 100 markers
...
for (crit in modern_criteria) {
  value <- criterion(train_unified, test_set, Wheat.M, criterion = crit)
...
cat("\nNormalized Criteria:\n")
cat("===================\n")
pev_norm <- criterion(train_unified, test_set, Wheat.M, criterion = "pev_mean_normalized")
cd_norm <- criterion(train_unified, test_set, Wheat.M, criterion = "cd_mean_normalized")
...
cat("====================\n")
aopt_modern <- criterion(train_unified, test_set, Wheat.M, criterion = "a_optimality")
aopt_legacy <- criterion(train_unified, test_set, Wheat.M, criterion = "AOPT")
...
for (i in 1:n_trials) {
  ...
  trial_fitness <- pev_mean(trial_solution, test_set, Wheat.M, normalized = TRUE)
...
random_fitness <- pev_mean(random_selection, test_set, Wheat.M, normalized = TRUE)
...
if (n_trials > 0) {
  ...
  for (i in 1:n_trials) {
    trial_solution <- sample(candidates, n_select)
    trial_results[i] <- pev_mean(trial_solution, test_set, Wheat.M, normalized = TRUE)
  }
...
if (length(pev_values) > 1 && length(cd_values) > 1) {
  ...
  random_cd <- cd_mean(random_selection, test_set, Wheat.M, normalized = TRUE)
...
P_train <- Wheat.M[train_stability, ]
...
cat("Number of markers:", ncol(Wheat.M), "\n")
...
# Test with extreme case (more predictors than observations)
train_extreme <- sample(candidates, 10)  # Very small training set
P_extreme <- Wheat.M[train_extreme, 1:100]  # Subset of markers
